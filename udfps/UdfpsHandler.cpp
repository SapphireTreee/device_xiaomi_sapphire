...
class XiaomiSm6225UdfpsHandler : public UdfpsHandler {
  public:
    ...
    void onFingerUp() {
        ...
        if (elapsed < 250) {
            // ENGLISH: Ignoring framework false UP
            LOG(INFO) << "UDFPS: Ignorando falso UP del framework (pasaron " << elapsed << "ms) / Ignoring framework false UP (elapsed " << elapsed << "ms)";
            return;
        }
  ...
  private:
    ...
    void screenStateMonitorThread() {
        ...
        while (isRunning.load()) {
            ...
            if (brightness != -1) {
                ...
                // ENGLISH: We read the status of the Android toggle switch
                bool isScreenOffEnabled = android::base::GetBoolProperty("persist.vendor.sys.fp.screen_off", true);

                if (currentState != lastState) {
                    // ENGLISH: If the switch is off, the monitor does NOT enable the touch screen elements
    ...
    void fodPressMonitorThread() {
        ...
        while (isRunning.load()) {
            ...
            // FILTRO DE FALSOS UP HARDWARE / HARDWARE FALSE UP FILTER
            if (pressed) {
                mFbDownTimeMs.store(now);
            } else {
                uint64_t elapsed = now - mFbDownTimeMs.load();
                if (elapsed < 250) {
                    LOG(INFO) << "UDFPS: Hardware marco UP muy rapido (" << elapsed << "ms). Esperando 100ms... / Hardware reported UP very rapidly (" << elapsed << "ms). Waiting 100ms...";
                    std::this_thread::sleep_for(std::chrono::milliseconds(100));
                    if (readBool(fd)) {
                        LOG(INFO) << "UDFPS: El dedo seguia ahi! Falso UP fisico ignorado. / The finger was still there! Physical false UP ignored.";
                        continue; 
                    }
                    LOG(INFO) << "UDFPS: El dedo se fue realmente. Forzando limpieza de LHBM. / The finger actually left. Forcing LHBM cleanup.";
                }
            }

            // SEGURIDAD: Solo manda el toque si Screen-Off esta activado o la pantalla encendida
            // SECURITY OVERRIDE: Only send touches if Screen-Off is active or the screen is turned on
            bool isScreenOffEnabled = android::base::GetBoolProperty("persist.vendor.sys.fp.screen_off", true);
            if (!isScreenOffEnabled && getBrightness() == 0) {
                LOG(INFO) << "UDFPS: Toque ignorado. Screen-Off desactivado. / Touch ignored. Screen-Off disabled.";
                // SPANISH: Si el dedo se levanto, aun queremos limpiar el estado aunque la pantalla este apagada
                // ENGLISH: Fallback: If the finger lifted, we still MUST clean up the state even if the screen is dark
                if (!pressed) {
                    setFingerDown(false);
                }
                continue;
