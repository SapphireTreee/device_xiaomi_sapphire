...
class XiaomiSm6225UdfpsHandler : public UdfpsHandler {
  public:
    void init(fingerprint_device_t* device) {
        ...
        if (touch_fd_.get() < 0) {
            LOG(ERROR) << "Failed to open " << TOUCH_DEV_PATH << " , err: " << strerror(errno);
            return;
        }

        disp_fd_ = android::base::unique_fd(open(DISP_FEATURE_PATH, O_RDWR));
        if (disp_fd_.get() < 0) {
            LOG(ERROR) << "Failed to open " << DISP_FEATURE_PATH << " , err: " << strerror(errno);
            return;
        }
        ...
        std::thread([this]() {
            ...
            if (fd.get() < 0) {
                LOG(ERROR) << "Failed to open " << FOD_PRESS_STATUS_PATH << " , err: " << strerror(errno);
                return;
            }
            ...
            while (true) {
                ...
                if (rc < 0) {
                    LOG(ERROR) << "Failed to poll " << FOD_PRESS_STATUS_PATH << ", err: " << strerror(errno);
                    continue;
                }
        ...
        std::thread([this]() {
            ...
            ioctl(fd.get(), MI_DISP_IOCTL_REGISTER_EVENT, &req);
            ...
            while (true) {
                ...
                if (rc < 0) {
                    LOG(ERROR) << "Failed to poll " << DISP_FEATURE_PATH << ", err: " << strerror(errno);
                    continue;
                }
